...
pub struct UpdateDiscussionCategoryRequest {
    ...
    pub description: Option<String>,
    pub format: Option<DiscussionCategoryFormat>,
    #[serde(default, deserialize_with = "deserialize_nullable_uuid_update")]
    pub section_id: Option<Option<Uuid>>,
}

fn deserialize_nullable_uuid_update<'de, D>(
    deserializer: D,
...
pub async fn repository_discussions_for_actor_by_owner_name(
    ...
    if let Some(active_label) = filters.label.as_deref() {
        labels.sort_by_key(|label| {
            (
                !label.name.eq_ignore_ascii_case(active_label),
...
pub async fn repository_discussion_detail_for_actor_by_owner_name(
    ...
    label_options.sort_by_key(|label| {
        (
            !labels.iter().any(|active| active.id == label.id),
            label.name.to_lowercase(),
        )
    });
...
async fn load_discussion_category_sections(
    ...
    let rows = sqlx::query(
        ...
        GROUP BY discussion_category_sections.id, discussion_category_sections.name, discussion_category_sections.position
