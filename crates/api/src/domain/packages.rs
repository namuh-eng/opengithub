...
fn push_visibility_predicate(
    ...
    match owner_kind {
        PackageOwnerKind::User => {
            builder.push(" OR p.owner_user_id = ");
            builder.push_bind(actor_user_id);
        }
        PackageOwnerKind::Organization => {
            builder.push(" OR EXISTS (SELECT 1 FROM organization_memberships om WHERE om.organization_id = p.owner_organization_id AND om.user_id = ");
            builder.push_bind(actor_user_id);
            builder.push(")");
        }
    }
    ...
    builder.push_bind(actor_user_id);
    builder.push(" AND pp.role IN ('read', 'write', 'admin'))");
    builder.push(" OR EXISTS (SELECT 1 FROM repository_permissions rp WHERE rp.repository_id = p.repository_id AND rp.user_id = ");
    builder.push_bind(actor_user_id);
