...
export default async function UserProjectRoute({
  params,
  searchParams,
}: UserProjectRouteProps) {
  ...
  const result = Number.isFinite(projectNumber)
    ? await getUserProjectWorkspace(ownerLogin, projectNumber, {
        view: firstParam(queryParams?.view),
    ...
    <AppShell session={session} shellContext={shellContext}>
      {result.ok ? (
        <ProjectWorkspacePage
          owner={ownerLogin}
          scope="user"
          viewNumber={result.workspace.selectedView.number}
