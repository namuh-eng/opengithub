...
type UserProjectWorkspaceRouteProps = {
  params: Promise<{ owner: string; number: string; view: string }>;
...
export default async function UserProjectWorkspaceRoute({
  params,
  searchParams,
}: UserProjectWorkspaceRouteProps) {
  const [{ owner, number, view }, queryParams, { session, shellContext }] =
  ...
  const result = Number.isFinite(projectNumber)
    ? await getUserProjectWorkspace(ownerLogin, projectNumber, {
        view,
