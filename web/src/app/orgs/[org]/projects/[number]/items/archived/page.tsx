...
type OrganizationProjectArchivedItemsRouteProps = {
...
export default async function OrganizationProjectArchivedItemsRoute({
  params,
  searchParams,
}: OrganizationProjectArchivedItemsRouteProps) {
  ...
  const workspaceResult = Number.isFinite(projectNumber)
    ? await getOrganizationProjectWorkspace(orgLogin, projectNumber, { view })
    : {
        ok: false as const,
        status: 404,
        code: "not_found",
