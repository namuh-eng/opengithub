...
import {
  type AccountSecurityLogFetchResult,
  ...
  type ProjectSettingsFetchResult,
...
export async function getAccountSecurityLog(
  query: AccountSecurityLogQuery = {},
): Promise<AccountSecurityLogFetchResult> {
  const requestHeaders = await headers();
  return getAccountSecurityLogFromCookie(requestHeaders.get("cookie"), query);
}
...
export async function getUserProjectSettings(
  ...
  const requestHeaders = await headers();
  return getUserProjectSettingsFromCookie(
    requestHeaders.get("cookie"),
    username,
    projectNumber,
  );
...
export async function getOrganizationProjectSettings(
  org: string,
  projectNumber: number,
): Promise<ProjectSettingsFetchResult> {
  const requestHeaders = await headers();
  return getOrganizationProjectSettingsFromCookie(
    requestHeaders.get("cookie"),
    org,
    projectNumber,
  );
...
export async function getRepositoryWikiPages(owner: string, repo: string) {
  const requestHeaders = await headers();
  return getRepositoryWikiPagesFromCookie(
    requestHeaders.get("cookie"),
    owner,
    repo,
  );
